 ...
 * 2. 监控指标：JSON解析通过率、标题合规率、标签数量、失败重试次数、自检触发率
 * 3. 预留回滚：如遇异常可一键回滚到v1.0版本
 * 4. 持续优化：每两周轮换口头禅池，每月复盘Top违反约束原因
 */
...
- **去AI味策略:**
  - **口头禅随机插入：** 从口头禅池中随机选择1-2个自然插入文中，避免重复使用
  - **句式节奏变化：** 混合使用2-8字短句与20-30字长句，允许使用半截句、括号插入和破折号
  - **长度抖动：** 正文字数在450-750字区间随机落点，根据素材信息量微调
  - **细节锚点强制抽取：** 从用户素材中强制抽取3-4个可验证细节（时间点、地点、价格、第三人原话），禁止凭空添加
  - **黑名单词规避：** 禁用"${AI_TEMPLATE_BLACKLIST.join('", "')}"等AI模板词
...
## 4. AI绘画提示词
(根据报告中coverStyleAnalysis.suggestion建议的封面风格和colorTone的色调偏好，创作一个生动的配图提示词。**重要：你的提示词必须引导AI生成一张适合3:4竖版比例的图片，这是小红书封面的黄金尺寸。**
**提示词格式要求：**
- 包含人物/物品类型（如"Chinese girl"、"skincare products"）
- 包含文字元素（如"bold Chinese text '平价逆袭'"）
- 使用coverStyleAnalysis.colorTone中指定的色调（如"bright warm tone"、"soft pastel colors"）
- 必须以"3:4 --ar 3:4"结尾
