...
- **呼吸感分段:** 拒绝"文字墙"！正文必须拆分成多个小段落，但长度要不规则。每句话都应该换行，确保阅读的美感。段落之间可以适当使用空行，制造阅读的"停顿"和"呼吸感"。
...
## 4. AI绘画提示词
(创作一个生动的图生图提示词。**重要：你的提示词仅能包含卖点文案的提示词。**
**提示词格式要求：**
- 仅允许包含文字元素（如"加粗字体 '平价逆袭'"）
- 因为是图生图，所以不需要任何场景有关的描述，只输出2-4个卖点即可
- 输出的提示词只包含卖点文案提示词，不要出现其他任何内容，不能影响AI的绘画
