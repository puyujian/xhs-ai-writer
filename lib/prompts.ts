...
{
  "titleFormulas": {
    "analysis": "分析这些标题最常用的模式。例如：'数字+结果'、'场景+人群'等。注意：所有公式都必须确保生成的标题控制在20字以内。",
    "suggestedFormulas": [
      "公式1: [数字] + [核心卖点] + [人群] (控制在20字内)",
      "公式2: [场景] + [解决方案] + [情感词] (控制在20字内)",
      "公式3: [品牌] + [新品] + [价格优势] (控制在20字内)",
      "公式4: [对比] + [选择建议] + [理由] (控制在20字内)",
      "公式5: [时间限定] + [行动指令] + [获益] (控制在20字内)"
    ],
...
- **标题字数限制：每个标题必须控制在20字以内，这是小红书平台的硬性规定**
